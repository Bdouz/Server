...
import server.model.content.ExchangeBoxes;
import server.model.content.ExchangeBoxes.boxInformation;
...
//			break;
		...
		case 11747:
			for (boxInformation i : ExchangeBoxes.boxInformation.values()) {
				if (itemId == i.getItemID() && c.getItems().freeSlots() >= 4) {
					...
					c.sendMessage("You successfully traded your set for its component items.");
				}
	...
	public static void ItemonItem(Client c, int itemUsed, int useWith) {
		...
		if (itemUsed == 11840 && useWith == 13231 || itemUsed == 13231
				&& useWith == 11840) {
			c.getDH().sendDialogues(328, -1);
		}
