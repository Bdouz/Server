...
public class PlayerAssistant {
	...
	public void destroySearch() {
		...
		sendNewString("The bank of Agatha.", 5383);
	}
	...
	public void searchBank(String str) {
		sendNewString("The bank of Agatha.", 5383);// didnt this change?
		c.items = new int[500];
		c.itemsN = new int[500];
		int p = 0;
		int slot = 0;
		...
		if (p > 0) {
			sendNewString("Bank of Agatha - (search: '" + str + "')", 5383);
	...
	public void loadQuests() {
		...
		sendNewString("Player Statistics:", 29161);
		sendNewString("@or1@Pk Points: @gre@" + c.pkPoints, 29169);
		if (c.KC == 0 && c.DC == 0) {
			sendNewString("@or1@KDR:@gre@ " + c.KC + "@or1@/@gre@" + c.DC, 29170);
		} else {
			sendNewString("@or1@KDR:@gre@ " + c.KC + "@yel@/@gre@" + c.DC, 29170);
		}
		sendNewString("@or1@Donator Points: @gre@" + c.donPoints, 29165);
		if (c.pcPoints == 0) {
			sendNewString("@or1@Pest-Control Points: @gre@" + c.pcPoints + "", 29163);
		} else {
			sendNewString("@or1@Pest-Control Points: @gre@" + c.pcPoints + "", 29163);
		}
		sendNewString("@or1@Vote Points: @gre@" + c.votPoints + "", 29164);
		...
		if (c.time != -1) {
			sendNewString("@or1@Double Exp:@gre@" + Client.parseTime((long) c.time), 29171);
		} else {
			sendNewString("@or1@Double Exp: @red@None", 29171);
		}
		sendNewString("", 29166);
		sendNewString("", 29172);
	
		
		  sendFrame126("@or1@Time Played:@gre@" 	+ c.getTimePlayed().formatPlayersTime(), 29162);
