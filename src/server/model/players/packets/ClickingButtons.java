...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Client c, int packetType, int packetSize) {
		...
		case 9192:
			...
			if (c.teleAction == 69) {
				c.getPA().spellTeleport(3044, 9788, 0);
			}
			if (c.teleAction == 11) {
		...
		case 9193:
			...
			if (c.teleAction == 69) {
				c.getPA().spellTeleport(2852, 3430, 0);
			}
			if (c.teleAction == 35) {
		...
		case 9157:// barrows tele to tunnels
			...
			if (c.dialogueAction == 328) {
				if (c.getItems().playerHasItem(13231)
						&& c.getItems().playerHasItem(11840)) {
					c.getItems().deleteItem(11840, 1);
					c.getItems().deleteItem(13231, 1);
					c.getItems().addItem(13239, 1);
					c.getPA().addSkillXP(22 * Config.RUNECRAFTING_EXPERIENCE,c.playerRunecrafting);
					c.getPA().addSkillXP(2 * Config.MAGIC_EXP_RATE,c.playerMagic);
					c.sendMessage("You Sucessfully infuse the Dragon boots and Primordial Crystal.");
					c.getPA().removeAllWindows();
				}
			}
			if (c.dialogueAction == 3516) {
				if (c.getItems().playerHasItem(995, 5000000)) {
					if (c.getItems().addItem(13280, 1)) {
						c.getItems().addItem(13281, 1);
						c.getItems().deleteItem(995, 5000000);
						c.sendMessage("You obtain the Max Cape and Hood for 5M coins.");
						c.sendGlobal("@cr6@@red@ Congratulations to "+c.playerName+ " for achieving 99 in all stats.");
					} else {
						c.sendMessage("Error completing transaction.");
					}
				} else {
					c.sendMessage("Sorry, You do not have enough coins to purchase this cape.");
				}
			}
		...
		// home teleports
		case 4171:
			c.getPA().startTeleport(3086, 3491, 0, "modern");
